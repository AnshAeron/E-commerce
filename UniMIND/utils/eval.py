    ...
    with torch.no_grad():
        ...
        if task in ["Understand", "Elicit", "Response"]:
            ...
            for _, samples in enumerate(tqdm(dataloader)):
                ...
                pad_tensor = torch.full(labels.shape, 0, device=labels.device)
                ...
                if output_crs_data:
                    for i, sample in enumerate(samples):
                        ...
                        if sample_id in merge_data_dict:
                            merge_data_dict[sample_id][f"{llm_mode}{args.base_name.replace('-', '_')}_crs_output"] = \
                            batch_output[i][1]
            ...
            for _, samples in enumerate(tqdm(dataloader)):
                ...
                if output_crs_data:
                    for i, sample in enumerate(samples):
                        sample_id = sample["sample_id"]
                        if sample_id in merge_data_dict:
                            merge_data_dict[sample_id][f"{llm_mode}{args.base_name.replace('-', '_')}_crs_output"] = rank[i]
