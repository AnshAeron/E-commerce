...
class BARTCRSModel(nn.Module):
    ...
    def forward(self, input_ids, attention_mask, labels=None, item_ids=None, llm_item_ids=None, sids=None):
        if item_ids is None:
            ...
            recommend_loss = self.recommend_criterion(score, torch.tensor(item_indexes).to(torch.device("cuda:0" if torch.cuda.is_available() else "cpu")))
    ...
    def fusion_historical_behaviors(self, sids, hidden_state):
        ...
        his_ids = torch.tensor([historical_action_id + [0] * (max_len - len(historical_action_id))
                                for historical_action_id in historical_action_ids]).to(torch.device("cuda:0" if torch.cuda.is_available() else "cpu")).long()
        his_mask = torch.tensor([[1] * len(historical_action_id) + [0] * (max_len - len(historical_action_id))
                                 for historical_action_id in historical_action_ids]).to(torch.device("cuda:0" if torch.cuda.is_available() else "cpu"))
    ...
    def recommend_item(self, input_ids, attention_mask, candidates, llm_item_ids=None, sids=None):
        # batch_size
        if llm_item_ids is not None:
            llm_item_indexes = [self.item_id2index.get(item_id, 0) for item_id in llm_item_ids]
            llm_missing_mask = torch.tensor(
                [item_id not in self.item_id2index for item_id in llm_item_ids],
                device=torch.device("cuda:0" if torch.cuda.is_available() else "cpu"),
            )
        else:
            llm_item_indexes = []
            llm_missing_mask = None

        # batch_size, 20
        candidates_index = [[self.item_id2index.get(item_id) for item_id in candidate] for candidate in candidates]
        ...
        if llm_item_ids is not None:
            llm_item_embeddings = self.item_embedding.weight[llm_item_indexes,]
            if llm_missing_mask is not None and llm_missing_mask.any():
                llm_item_embeddings = llm_item_embeddings.clone()
                llm_item_embeddings[llm_missing_mask] = 0
            user_representation = user_representation + llm_item_embeddings

        ranks = []
        for i, candidate_index in enumerate(candidates_index):
            valid_pairs = [(item_id, idx) for item_id, idx in zip(candidates[i], candidate_index) if idx is not None]
            if valid_pairs:
                valid_item_ids, valid_indexes = zip(*valid_pairs)
                candidate_embedding = self.item_embedding.weight[list(valid_indexes),].transpose(0, 1)
                score_tensor = torch.matmul(user_representation[i], candidate_embedding) + self.item_bias.bias[list(valid_indexes),]
                score_map = {item_id: score_tensor[j].item() for j, item_id in enumerate(valid_item_ids)}
            else:
                score_map = {}
            rank = sorted(candidates[i], key=lambda x: score_map.get(x, float("-inf")), reverse=True)
            ranks.append(rank)
    ...
    def forward(self, input_ids, attention_mask, labels=None, item_ids=None, llm_item_ids=None, sids=None):
        if item_ids is None:
            ...
            valid_mask = torch.tensor(
                [item_id in self.item_id2index for item_id in item_ids],
                device=input_ids.device,
            )
            if valid_mask.sum().item() == 0:
                return torch.zeros((), device=input_ids.device, requires_grad=True)

            input_ids = input_ids[valid_mask]
            attention_mask = attention_mask[valid_mask]
            item_ids = [item_id for item_id, keep in zip(item_ids, valid_mask.tolist()) if keep]
            if sids is not None:
                sids = [sid for sid, keep in zip(sids, valid_mask.tolist()) if keep]

            item_indexes = [self.item_id2index[item_id] for item_id in item_ids]
            if llm_item_ids is not None:
                llm_item_ids = [item_id for item_id, keep in zip(llm_item_ids, valid_mask.tolist()) if keep]
                llm_item_indexes = [self.item_id2index.get(item_id, 0) for item_id in llm_item_ids]
                llm_missing_mask = torch.tensor(
                    [item_id not in self.item_id2index for item_id in llm_item_ids],
                    device=input_ids.device,
